...
class AdminProvider extends ChangeNotifier {
  ...
  Future<void> fetchPendingRequests() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('${AppConfig.baseUrl}/destinations/pending'), // Centralized Base URL
        headers: headers,
      );
  ...
  Future<void> fetchAnalytics() async {
    ...
    try {
      final headers = await _getHeaders();
      final response = await http.get(
        Uri.parse('${AppConfig.baseUrl}/analytics'), // Centralized Base URL
        headers: headers,
      );
      if (response.statusCode == 200) {
  ...
  Future<void> approveDestination(String id) async {
    try {
      ...
      final response = await http.patch(
        Uri.parse('${AppConfig.baseUrl}/destinations/$id/approve'), // Centralized Base URL
        headers: headers,
      );
      if (response.statusCode == 200) {
  ...
  Future<void> rejectDestination(String id) async {
    try {
      ...
      final response = await http.delete(
        Uri.parse('${AppConfig.baseUrl}/destinations/$id/reject'), // Centralized Base URL
        headers: headers,
      );
      if (response.statusCode == 200) {
        _pendingRequests.removeWhere((req) => req['_id'] == id);
